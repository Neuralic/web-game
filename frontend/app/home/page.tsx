...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchRecentlyPlayed = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/games/recently-played/${user.id}`);
        const data = await res.json();
        if (data.success && data.data?.games) {
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    const res = await feedApi.deletePost(postId);
    if (res.success) {
                ...
                {friends.map((friend) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs font-semibold text-gray-900 dark:text-gray-100">{friend.name}</p>
                    </Link>
                  );
                })}
              </div>
